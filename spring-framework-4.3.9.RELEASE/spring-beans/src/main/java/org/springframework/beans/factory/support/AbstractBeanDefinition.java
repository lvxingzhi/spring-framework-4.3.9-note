...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	public boolean isSynthetic() {
		return this.synthetic;// 是否是用户创建的
	}
	...
	protected void prepareMethodOverride(MethodOverride mo) throws BeanDefinitionValidationException {
		int count = ClassUtils.getMethodCountForName(getBeanClass(), mo.getMethodName());// 方法在class中的个数
		if (count == 0) {// 不存在覆盖的方法,抛异常,设置有错误
			throw new BeanDefinitionValidationException(
					"Invalid method override: no method with name '" + mo.getMethodName() +
					"' on class [" + getBeanClassName() + "]");
		}
		else if (count == 1) {// 存在
			// Mark override as not overloaded, to avoid the overhead of arg type checking. // 标记为不用类型检查,将来被覆盖,没必要检查
