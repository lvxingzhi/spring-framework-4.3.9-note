...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	public int loadBeanDefinitions(Resource resource) throws BeanDefinitionStoreException {
		return loadBeanDefinitions(new EncodedResource(resource));// 带编码设置的资源对象
	}
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		// 重复资源文件校验
		if (!currentResources.add(encodedResource)) {// 如果重复加载文件,抛出异常
		...
		try {
			...
			try {
				...
				if (encodedResource.getEncoding() != null) {// 如果设置了编码,则赋值
	...
	protected Document doLoadDocument(InputSource inputSource, Resource resource) throws Exception {
		// 资源,本地校验文件位置,错误处理器,校验模式(DTD OR XSD),是否处理命名空间,这里用到sax只是API的重用,底层为DOMAPI
		// 参考: http://www.cnblogs.com/xing901022/p/4565730.html
		// 文档读取器读取资源
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		// 创建Spring的Document读取器
		BeanDefinitionDocumentReader documentReader = createBeanDefinitionDocumentReader();
		// getRegistry() 注册器,相当于注册表或者缓存的功能
		// 记录本次加载前已加载的bean数量
		int countBefore = getRegistry().getBeanDefinitionCount();
		// XmlReaderContext Xml读取上下文,存放xml解析使用的对象,读取监听器,读取状态等,自定义标签解析器
		// 读取器读取
	...
	public XmlReaderContext createReaderContext(Resource resource) {
		// 资源,异常处理器,空的监听器,空的资源提取器,this,标签解析器
